...
def get_NickName(url):
    ...
    nickname = _nickname_re.search(res.text).group(1)
    print(nickname)
    return nickname
...
class CrawlerScheduler(object):
    ...
    def download_user_videos(self, url):
        ...
        if not len(number):
            return
        dytk = get_dytk(url)
        nickname = get_NickName(url)
        hostname = urllib.parse.urlparse(url).hostname
        if hostname != 't.tiktok.com' and not dytk:
            return
        user_id = number[0]
        video_count = self._download_user_media(user_id, dytk, url, nickname)
        self.queue.join()
    ...
    def _download_user_media(self, user_id, dytk, url, nickname):
        current_folder = os.getcwd()
        target_folder = os.path.join(current_folder, 'download/%s' % nickname)
