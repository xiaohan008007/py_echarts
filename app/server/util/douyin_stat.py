...
def get_last_2_week(start, end):
    ...
    for add_day in range(start, end):
        ...
        # print(yesterday)
        filename = "audit_douyin/audit_douyin_cid.txt." + yesterday
...
def get_stat_content():
    ...
    x = PrettyTable(["打标人",  "昨日", "前日", "本周", "上周", "今日"])
    v0_total = 0
    v1_total = 0
    v2_total = 0
    v3_total = 0
    ...
    for k, v in user_tree.items():
        x.add_row([k, v[0], v[1], v[2], v[3], v[4]])
        v0_total += v[0]
        v1_total += v[1]
        v2_total += v[2]
        v3_total += v[3]
        v4_total += v[4]
    x.add_row(['总计', v0_total, v1_total, v2_total, v3_total, v4_total])
